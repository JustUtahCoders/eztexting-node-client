  ...
  for (let contact of contacts) {
    ...
    if (clientDuplicates.length === 0 && leadDuplicates.length === 0) {
      ...
      if (clientDuplicates.length > 0) {
        const client = clientDuplicates[0];
        console.log(
          `--> Found duplicate. Automatically updating client ${
            client.firstName
          } ${client.lastName}. ${getBaseUrl()}/clients/${client.id}`
        );
        await updateClient(contact, client.id);
        contact.cuDatabaseClientId = client.id;
      } else {
        const lead = leadDuplicates[0];
        console.log(
          `--> Found duplicate. Automatically updating lead ${lead.firstName} ${
            lead.lastName
          }. ${getBaseUrl()}/leads/${lead.id}`
        );
        await updateLead(contact, lead.id);
        contact.cuDatabaseLeadId = lead.id;
      }
    }
    // else {
    ...
    //         client.lastName
    //       } - ${client.phone} - ${getBaseUrl()}/clients/${client.id}`
    //     )
    //   );
    ...
    //         lead.lastName
    //       } - ${lead.phone} - ${getBaseUrl()}/leads/${lead.id}`
    //     )
    //   );
    //   console.log(
